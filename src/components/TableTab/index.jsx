...
function TableTab(props) {
  ...
  const getScroll = () => {
    const height = tableTabRef?.current?.offsetHeight;
  ...
  return (
      <div
        ref={tableTabRef}
        className={`${styles.tableTab} ${!border ? '' : styles.borderTable} ${line ? styles.line_margin : ''} ${styles[`height${heightType}`]} `}
        style={style}
      >
          {TabsList.length > 0 && <ul className={styles.tabs_titles}>

            {
              TabsList.map((item,indx )=> <li key={indx}  className={activeKey === item.key ? styles.active : ''} onClick={() => changeTabs && changeTabs(item.key)}> <img src={activeKey === item.key ? item.iconA: item.icon} alt='' /> {item.title}</li>)
            }
          </ul>}
          ...
          <div className={styles.tableTabHead}>
            ...
            {cilckTitle && !sessionStorage.getItem('area') && (
              <span onClick={()=>{setRowId('北京');cilckTitle('北京')}} className={styles.title}>
                北京
              </span>
            )}
